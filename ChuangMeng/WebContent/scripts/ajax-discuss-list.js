define(function(require,exports){
	...
	function loaded(data){
		if(data.page_total==0&&!data.list.length){
			$itemContainer.html("<p class='unquestion'><span></span>此课程暂无同学的讨论</p>");
			return false;
		}
			...
			$itemContainer.before("<div class='sortlist'> \
				<div class='ordertab'> \
					<a href='javascript:void(0)' hidefocus='true' data-order='last' class='"+(!order||order=="last"?"newOn":"")+"'>最新</a>\
					<a href='javascript:void(0)' hidefocus='true' data-order='pop' class='"+(order=="pop"?"hotOn":"")+"'>热门</a>\
				</div>\
			</div>");
			...
			$(document).on("click","[data-order]",function(e){
				e.preventDefault();
				var $this=$(this),
					cls=$this.text()=="热门"?"newOn":"hotOn",
					addCls=cls=="newOn"?"hotOn":"newOn";
				if($this.hasClass(cls)){
					return ;
				}
				$this.siblings("."+cls).removeClass(cls).end().addClass(addCls);
